 ...
 */
export function CodeSnippetInputDialog(codeSnippetWidget, code, idx) {
    const tags = [];
        ...
        if (validateForm(result) === false) {
            ...
            for (const snippet of currSnippets) {
                if (snippet.name === newSnippet.name) {
                    const result = saveOverWriteFile(codeSnippetWidget.codeSnippetWidgetModel, snippet, newSnippet);
                    result
                        .then(newSnippets => {
...
function createNewSnippet(codeSnippet, newSnippet, contentsService) {
    const request = contentsService.save('snippets/' + newSnippet.name + '.json', {
        type: 'file',
        format: 'text',
 ...
 */
async function saveOverWriteFile(codeSnippetWidgetModel, oldSnippet, newSnippet) {
 ...
 */
async function shouldOverwrite(path) {
    const options = {
...
class InputHandler extends Widget {
    ...
    getValue() {
        const inputs = [];
        inputs.push(this.node.getElementsByTagName('input')[0].value, this.node.getElementsByTagName('input')[1].value, this.node.getElementsByTagName('input')[2].value);
...
class Private {
    ...
    static createConfirmMessageNode() {
